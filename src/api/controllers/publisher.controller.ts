...
export default class PublisherController {
  constructor(
    ...
    private repositoryBook?: Repository<Book>,
  ) {}
  ...
  async getPublisherById(req: Request, res: Response) {
    ...
    try {
      const Publisher = await this.context.findOne({
  ...
  async createOrUpdatePublisher(req: Request, res: Response) {
    ...
    try {
      if (pathname === "add") {
        const isPublisherExist = await this.context.findOne({
          name: publisher.name,
        });

        if (isPublisherExist)
          return res.status(404).json({ message: "A Editora já existe" });
        const newPublisher = Publisher.create(publisher);
      ...
      if (!isPublisher) {
        console.log(
          "🚀 ~ PublisherController ~ createOrUpdatePublisher ~ isBookExist:",
          isPublisher,
        );
        return;
      }
